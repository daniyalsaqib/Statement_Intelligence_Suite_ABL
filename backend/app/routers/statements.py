...
UPLOAD_TOO_LARGE_DETAIL = "Uploaded CSV file is too large."

TOO_MANY_TRANSACTIONS_DETAIL = "Uploaded CSV contains too many transactions."
...
def _parse_uploaded_statement(
    ...
    try:
        return parse_statement_csv(content)

    except StatementParseError as exc:
        raise HTTPException(
            status_code=400,
            detail=("Could not parse statement: " f"{str(exc)}"),
        ) from exc

    except Exception as exc:
        logger.error(
            "Unexpected statement parser failure: " "exception_type=%s",
            type(exc).__name__,
        )
...
def _enforce_transaction_limit(
    ...
    if len(transactions) > MAX_STATEMENT_ROWS:
        raise HTTPException(
            status_code=413,
            detail=(TOO_MANY_TRANSACTIONS_DETAIL),
        )
...
async def upload_statement(
    ...
    transactions = _parse_uploaded_statement(content)

    _enforce_transaction_limit(transactions)
    ...
    try:
        ...
        logger.error(
            "Unexpected statement analysis failure: " "exception_type=%s",
            type(exc).__name__,
        )
...
async def analyze_subscriptions(
    ...
    try:
        ...
        logger.error(
            "Unexpected recurring-payment " "analysis failure: exception_type=%s",
            type(exc).__name__,
        )

        raise HTTPException(
            status_code=500,
            detail=(SUBSCRIPTION_PROCESSING_ERROR_DETAIL),
        ) from exc
