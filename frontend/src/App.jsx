...
function App() {
  ...
  // MULTI-TURN STATEMENT CHAT:

  //

  // Purana UI sirf aik statementAnswer rakhta tha,

  // isliye har naya answer previous answer ko replace kar deta tha.

  //

  // Ab poori visible conversation role-based messages mein rahegi.

  const [statementMessages, setStatementMessages] = useState([]);
  ...
  async function analyzeStatement() {
    ...
    // Nayi statement ka apna fresh conversation context hoga.

    // Purani statement ki chat kabhi new statement ke saath mix nahi honi chahiye.

    setStatementMessages([]);
  ...
  async function askStatementQuestion() {
    ...
    const userQuestion = statementQuestion.trim();
    ...
    // Current question request.question mein separately ja rahi hai.

    //

    // Isliye conversation_history mein current question dobara

    // include nahi karte. Sirf previous conversation bhejte hain.

    //

    // Backend maximum 8 history messages accept karta hai,

    // isliye frontend bhi wahi boundary enforce karta hai.

    const conversationHistory = statementMessages.slice(-8).map((message) => ({
    ...
    // USER MESSAGE KO UI MEIN IMMEDIATELY SHOW KARO:

    //

    // Isse chat responsive feel karti hai.

    // Backend response ka wait kiye baghair user apna

    // submitted question conversation mein dekh sakta hai.

    setStatementMessages((currentMessages) => [
    ...
    // Composer ko next question ke liye clear kar dete hain.
    ...
    try {
      ...
      // Previous messages preserve rehte hain.

      // Naya answer conversation ke end mein append hota hai.
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-5 rounded-2xl border border-blue-100 bg-[#f8fbff] p-5 sm:p-6">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          // New statement = new conversation.
...
----------------------------------------------------- */}
                    ...
                    {/* -----------------------------------------------------
    CONVERSATIONAL STATEMENT ASSISTANT

    Purana version sirf latest answer show karta tha.

    Ab user aur assistant dono ke messages preserve hote hain,
    isliye proper Q1 -> A1 -> Q2 -> A2 flow possible hai.
----------------------------------------------------- */}
                    <div className="mt-9 rounded-2xl border border-slate-200 bg-[#f8fafc] p-5 sm:p-6">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <h3 className="mt-1 font-extrabold text-[#063b6f]">
                            Ask about this statement
                          </h3>

                          <p className="mt-1 text-sm leading-6 text-slate-500">
                            Ask a question, then continue with follow-ups using
                            the same uploaded statement.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-6 space-y-4">
                        {statementMessages.length === 0 ? (
                          <div className="rounded-xl border border-dashed border-slate-200 bg-white px-4 py-5 text-sm leading-6 text-slate-500">
                            <p className="font-semibold text-slate-700">
                              Start a conversation about the statement.
                            </p>

                            <p className="mt-1">
                              Example: “How much did I spend in August?” Then
                              ask: “What about July?”
                            </p>
                          </div>
                        ) : (
                          statementMessages.map((message, index) => (
                            <div
                        ...
                        {loading && (
                          <div className="flex justify-start">
                            <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                              ...
                              <p className="mt-1 text-sm text-slate-500">
                                Analyzing verified statement data...
                              </p>
                            </div>
                          </div>
                        )}
                      ...
                      <div className="mt-6 border-t border-slate-200 pt-5">
                        <textarea
                          ...
                          placeholder={
                            statementMessages.length === 0
                              ? "Example: How much did I spend in August?"
                              : "Ask a follow-up, for example: What about July?"
                          }
                          className="abl-input"
                          disabled={loading}
                        />
                        ...
                        {/* TRUST / PROVENANCE NOTE:
                            Conversation improves context,
                            lekin financial calculations backend statement data
                            se hi verify hoti hain. */}
                        <div className="mt-5 rounded-xl border border-blue-100 bg-blue-50/60 px-4 py-3">
                          <p className="text-xs leading-5 text-slate-600">
                            <span className="font-bold text-[#063b6f]">
                              Verified statement context:
                            </span>{" "}
                            Follow-up conversation helps interpret your
                            question, while financial figures continue to come
                            from the uploaded statement and backend
                            calculations.
                          </p>
                        </div>
                      </div>
